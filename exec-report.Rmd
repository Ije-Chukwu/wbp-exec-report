...
#### IPV: Survey Responses by Country to Have you Ever been Slapped by an intimate partner (last 12 months)
...
df <- read_excel("slapped in last 12 months freq no missing values.xlsx", sheet = "slapped in last 12 months freq")

# List the columns you want in the proportion calculation (include 'Missing')
response_cols <- c(
  "Not ever slapped",
...
for (col in response_cols) {
  df[[paste0(col, " %")]] <- 100 * df[[col]] / df$Total
}
...
fig <- plot_ly(
  ...
  layout(
    barmode = "stack",
    title = "“Slapped in Last 12 Months",
    xaxis = list(title = "Country", tickangle = -45),   # Rotate x labels if crowded
