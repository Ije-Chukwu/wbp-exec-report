...
clean_spss_output <- function(df_raw) {
# Set row 3 as header
  new_header <- as.character(unlist(df_raw[3, ]))
  colnames(df) <- new_header  # Assign new column names

  df %>%
    # Remove rows 1-2 and last row in cleaned table
    slice(-(1:2), -n()) %>%
    # Remove columns 1 and last column
    select(-1, -ncol(.)) %>%
    # Rename column 2 to 'country'
    rename(country = 2) %>%
    # Remove columns named 'Missing' and 'Not in Universe col' if present
    select(-any_of(c("Missing", "Not in Universe col")))
}
